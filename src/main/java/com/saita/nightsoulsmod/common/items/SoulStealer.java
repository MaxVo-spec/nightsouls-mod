...
public class SoulStealer extends SwordItem {
    ...
    tooltip.add(new StringTextComponent("§7Restore your health by reaping the souls of living beings. Only §5Miizeal§7 can hold this weapon."));
	...
	public boolean hitEntity(ItemStack stack, LivingEntity target, LivingEntity attacker) {
		int healAmmount = random.nextInt(1);
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
		...
		if(entityIn instanceof PlayerEntity)
    	{
    	   ...
    	   if(!playerIn.inventory.hasItemStack(new ItemStack(ItemInit.MIIZEAL_SOUL.get())))
    	   {
    		  ...
    		  if(((playerIn.getHeldItemMainhand().getItem() == ItemInit.SOUL_STEALER.get())) || 
    			(playerIn.getHeldItemOffhand().getItem() == ItemInit.SOUL_STEALER.get()))
